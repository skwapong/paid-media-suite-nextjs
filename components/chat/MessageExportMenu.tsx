...
const MessageExportMenu: React.FC<MessageExportMenuProps> = ({ message, disabled = false }) => {
  ...
  const exportAsPDF = () => {
    ...
    doc.save(`agent-response-${timestamp}.pdf`)
    setIsOpen(false)
  }
  ...
  const exportAsMarkdown = () => {
    ...
    const timestamp = new Date().toISOString().split('T')[0]
    downloadExport(content, `agent-response-${timestamp}.md`, 'text/markdown')
    setIsOpen(false)
  }
  ...
  const exportAsText = () => {
    ...
    const timestamp = new Date().toISOString().split('T')[0]
    downloadExport(content, `agent-response-${timestamp}.txt`, 'text/plain')
    setIsOpen(false)
  }
  ...
  const exportAsJSON = () => {
    ...
    const timestamp = new Date().toISOString().split('T')[0]
    downloadExport(content, `agent-response-${timestamp}.json`, 'application/json')
    setIsOpen(false)
  }
...
<body>
  ...
  <div class="footer">
    ...
    const timestamp = new Date().toISOString().split('T')[0]
    downloadExport(html, `agent-response-${timestamp}.html`, 'text/html')
    setIsOpen(false)
  }
